...
async function createMedicalHistory(req, res, next) {
    try {
        const { RetailEnrolleeDependentMedicalHistory, RetailEnrolleeDependent, Provider, Diagnosis } = req.models;
        ...
        const medicalHistory = await RetailEnrolleeDependentMedicalHistory.create({
            retailEnrolleeDependentId: dependentId,
        ...
        await addAuditLog(req.models, {
            action: 'retailDependentMedicalHistory.created',
...
async function getMedicalHistories(req, res, next) {
    try {
        const { RetailEnrolleeDependentMedicalHistory, Provider, Diagnosis } = req.models;
        const { dependentId } = req.params;
        ...
        const where = { retailEnrolleeDependentId: dependentId };

        if (status) {
            where.status = status;
        }

        const { count, rows } = await RetailEnrolleeDependentMedicalHistory.findAndCountAll({
            where,
            include: [
...
async function updateMedicalHistory(req, res, next) {
    try {
        const { RetailEnrolleeDependentMedicalHistory, RetailEnrolleeDependent } = req.models;
        ...
        const medicalHistory = await RetailEnrolleeDependentMedicalHistory.findOne({
            where: { id: medicalHistoryId, retailEnrolleeDependentId: dependentId }
        });
        ...
        // Add audit log
        await addAuditLog(req.models, {
            action: 'retailDependentMedicalHistory.updated',
...
async function deleteMedicalHistory(req, res, next) {
    try {
        ...
        // Add audit log
        await addAuditLog(req.models, {
            action: 'retailDependentMedicalHistory.deleted',
