...
async function listNotifications(req, res, next) {
    try {
        const { RetailEnrolleeNotification } = req.models;
        const { retailEnrolleeId, limit = 50, page = 1, isRead, notificationType } = req.query;

        if (!retailEnrolleeId) {
            return res.fail('`retailEnrolleeId` is required in query parameters', 400);
        }
        ...
        const { count, rows } = await RetailEnrolleeNotification.findAndCountAll({
            where,
...
async function updateNotificationStatus(req, res, next) {
    try {
        ...
        const { retailEnrolleeId } = req.query;
        ...
        const result = await RetailEnrolleeNotification.update({ isRead: readValue }, { where });
        const affected = Array.isArray(result) ? result[0] : result;

        const updated = await RetailEnrolleeNotification.findAll({ where });
...
async function getUnreadCount(req, res, next) {
    try {
        ...
        const count = await RetailEnrolleeNotification.count({
            where: { retailEnrolleeId, isRead: false }
        });
