...
// Controller for provider account operations: update profile (excluding email) and change password
const updateProfile = () => async (req, res, next) => {
    try {
        ...
        const Provider = req.models && req.models.Provider;
        if (!Provider) return res.fail('Server configuration error (Provider model missing)', 500);

        const user = await Provider.findByPk(userId);
        if (!user) return res.fail('Provider not found', 404);
