...
import EditPaymentAdvice from "@/components/pages/paymentAdvice/editPaymentAdvice";
...
const PaymentAdviceTable: React.FC = () => {
  ...
  const [editingAdvice, setEditingAdvice] = useState<PaymentAdvice | null>(
    null
  );
  const removePaymentAdvice = usePaymentAdviceStore(
    (s) => s.removePaymentAdvice
  );
  const [errorMessage, setErrorMessage] = useState(
    "Failed to delete payment advice. Please try again."
  );
  ...
  const headers: Header[] = [
    { key: "paymentAdviceNumber", label: "Advice Number" },
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const url = `/admin/payment-advices/list?${params.toString()}`;
  ...
  }, [limit, currentPage, search, setPaymentAdvices]);
  ...
  const handleView = (advice: PaymentAdvice) => {
    setEditingAdvice(advice);
    openModal();
  };

  const deletePaymentAdvice = async () => {
    if (!selectedAdviceId) return;
    try {
      setLoading(true);
      const url = `/admin/payment-advices/${selectedAdviceId}`;
      await apiClient(url, {
        method: "DELETE",
        onLoading: (l) => setLoading(l),
      });

      removePaymentAdvice(selectedAdviceId);
      setSelectedAdviceId(null);
      confirmModal.closeModal();
  ...
  const handleNavigate = (advice: PaymentAdvice) => {
    router.push(`/payment-advice/${advice.id}`);
  };
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full table-auto">
            ...
            <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
              {paymentAdvices.map((advice: PaymentAdvice) => (
                <tr
                  key={advice.id}
                ...
                >
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {advice.provider?.name ||
                        advice.provider?.code ||
                        "Unknown"}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    ...
                    >
                      {capitalizeWords(advice.status)}
                    </span>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {typeof advice.totalAmount === "number"
                        ? `$${advice.totalAmount.toFixed(2)}`
                        : "$0.00"}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {advice.paymentDate
                        ? formatDate(advice.paymentDate)
                        : "-"}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {advice.dueDate ? formatDate(advice.dueDate) : "-"}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {advice.createdAt ? formatDate(advice.createdAt) : "-"}
                    </p>
                  </td>
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <div className="flex items-center w-full gap-2">
                      ...
                      <button
                        onClick={() => handleDeleteModal(advice.id)}
      ...
      <ConfirmModal
        ...
        handleSave={deletePaymentAdvice}
