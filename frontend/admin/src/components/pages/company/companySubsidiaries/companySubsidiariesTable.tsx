...
interface CompanySubsidiariesTableProps {
  companyId: string;
}

const CompanySubsidiariesTable: React.FC<CompanySubsidiariesTableProps> = ({
  companyId,
}) => {
  ...
  const subsidiaries = useCompanySubsidiaryStore((s) => s.subsidiaries);
  const setSubsidiaries = useCompanySubsidiaryStore((s) => s.setSubsidiaries);
  const confirmModal = useModal();
  const [selectedSubsidiaryId, setSelectedSubsidiaryId] = useState<
    string | null
  >(null);
  const [editingSubsidiary, setEditingSubsidiary] =
    useState<CompanySubsidiary | null>(null);
  const removeSubsidiaryFromStore = useCompanySubsidiaryStore(
    (s) => s.removeSubsidiary
  );
  const [errorMessage, setErrorMessage] = useState(
    "Failed to delete company subsidiary. Please try again."
  );

  type Header = {
    key: keyof CompanySubsidiary | "actions";
    label: string;
  };
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      console.warn("Company subsidiaries fetch failed", err);
    } finally {
      setLoading(false);
    }
  }, [limit, currentPage, search, companyId, setSubsidiaries]);
  ...
  const handleView = (subsidiary: CompanySubsidiary) => {
  ...
  const deleteSubsidiary = async () => {
    try {
      ...
      await deleteCompanySubsidiary(selectedSubsidiaryId);
      removeSubsidiaryFromStore(selectedSubsidiaryId);
      ...
      setErrorMessage(
        err instanceof Error
          ? err.message
          : "Failed to delete company subsidiary"
      );
            ...
            <div className="hidden flex-col gap-3 sm:flex sm:flex-row sm:items-center">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search..."
                  className="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pr-4 pl-4 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden xl:w-[300px] dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                  value={search}
              ...
              <button
                onClick={openModal}
                className="cursor-pointer bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium text-white transition"
              >
                <svg
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full table-auto">
              ...
              <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
                {subsidiaries.map((subsidiary: CompanySubsidiary) => (
                  <tr
                    key={subsidiary.id}
                  ...
                  >
                    <td className="p-4 whitespace-nowrap">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-400">
                        {capitalizeWords(subsidiary.name)}
                      </span>
                    </td>
                    <td className="p-4 whitespace-nowrap">
                    ...
                    <td className="p-4 whitespace-nowrap">
                      <div className="flex items-center w-full gap-2">
                        ...
                        <button
                          onClick={() => handleDeleModal(subsidiary.id)}
...
export default CompanySubsidiariesTable;
