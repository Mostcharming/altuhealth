...
interface ViewCompanyPlanModalProps {
  isOpen: boolean;
  closeModal: () => void;
...
const ViewCompanyPlanModal: React.FC<ViewCompanyPlanModalProps> = ({
  isOpen,
  closeModal,
  plan,
}) => {
  ...
  const fetchSelectedItems = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiClient(
        `/admin/company-plans/${plan.id}?include=benefitCategories,exclusions,providers`,
        {
          method: "GET",
        }
      );
      ...
      >
        <div className="px-2 mb-6 ">
          <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
            {plan?.name || "Company Plan"} - Manage Resources
          </h4>
...
export default ViewCompanyPlanModal;
