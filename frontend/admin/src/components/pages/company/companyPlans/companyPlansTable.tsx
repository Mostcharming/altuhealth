...
import { deleteCompanyPlan, fetchCompanyPlans } from "@/lib/apis/companyPlan";
...
import ViewCompanyPlanModal from "./viewCompanyPlanModal";

interface CompanyPlansTableProps {
  companyId: string;
}

const CompanyPlansTable: React.FC<CompanyPlansTableProps> = ({ companyId }) => {
  ...
  const plans = useCompanyPlanStore((s) => s.plans);
  const setPlans = useCompanyPlanStore((s) => s.setPlans);
  const confirmModal = useModal();
  const [selectedPlanId, setSelectedPlanId] = useState<string | null>(null);
  const [editingPlan, setEditingPlan] = useState<CompanyPlan | null>(null);
  const [viewingPlan, setViewingPlan] = useState<CompanyPlan | null>(null);
  const removePlanFromStore = useCompanyPlanStore((s) => s.removePlan);
  const [errorMessage, setErrorMessage] = useState(
    "Failed to delete company plan. Please try again."
  );

  type Header = {
    key: keyof CompanyPlan | "actions";
    label: string;
  };
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      console.warn("Company plans fetch failed", err);
    } finally {
      setLoading(false);
    }
  }, [limit, currentPage, search, companyId, setPlans]);
  ...
  const handleView = (plan: CompanyPlan) => {
    setEditingPlan(plan);
    openModal();
  };

  const handleViewResources = (plan: CompanyPlan) => {
    setViewingPlan(plan);
    viewModal.openModal();
  };
  ...
  const deletePlan = async () => {
    try {
      ...
      await deleteCompanyPlan(selectedPlanId);
      removePlanFromStore(selectedPlanId);
      ...
      setErrorMessage(
        err instanceof Error ? err.message : "Failed to delete company plan"
      );
              ...
              <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
                {plans.map((plan: CompanyPlan) => (
                  ...
                  >
                    ...
                    <td className="p-4 whitespace-nowrap">
                      <p className="text-sm text-gray-700 dark:text-gray-400">
                        {capitalizeWords(plan.planCycle)}
                      </p>
                    </td>
                    <td className="p-4 whitespace-nowrap">
                      <p className="text-sm text-gray-700 dark:text-gray-400">
