...
const ProviderTable: React.FC = () => {
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const url = `/admin/providers/list?${params.toString()}`;
  ...
  }, [limit, currentPage, search, setProviders]);
  ...
  const deleteProvider = async () => {
    ...
    try {
      setLoading(true);
      const url = `/admin/providers/${selectedProviderId}`;
      await apiClient(url, {
        method: "DELETE",
        onLoading: (l) => setLoading(l),
      });

      removeProvider(selectedProviderId);
      setSelectedProviderId(null);
      confirmModal.closeModal();
  ...
  const getPlansCount = (provider: Provider) => {
    return provider.Plans ? provider.Plans.length : 0;
  };
                ...
                >
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <div className="flex items-center w-full gap-2">
                      ...
                      <button
                        onClick={() => handleDeleModal(provider.id)}
