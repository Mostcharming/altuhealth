...
const ApprovalsTable: React.FC = () => {
  ...
  const [selectedApproval, setSelectedApproval] =
    useState<AdminApproval | null>(null);
  const [statusFilter, setStatusFilter] = useState<string>("");
  const [errorMessage, setErrorMessage] = useState("");

  const approvals = useApprovalsStore((s) => s.approvals);
  const setApprovals = useApprovalsStore((s) => s.setApprovals);
  ...
  useEffect(() => {
    const fetchApprovals = async () => {
      try {
        ...
        const url = `/admin/approvals/list?${params.toString()}`;
        ...
        const totalPagesCalculated = Math.ceil(count / limit);
    ...
    fetchApprovals();
  }, [currentPage, limit, statusFilter, setApprovals]);
  ...
  const handleViewDetails = (approval: AdminApproval) => {
                    ...
                    >
                      ...
                      <td className="p-4 whitespace-nowrap text-center">
                        <button
                          ...
                          className="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90 inline-flex items-center justify-center"
                        >
                          <EyeIcon />
                        </button>
                      </td>
