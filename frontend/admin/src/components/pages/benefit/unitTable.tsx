...
import {
  BenefitCategory,
  useBenefitCategoryStore,
} from "@/lib/store/benefitCategoryStore";
...
const AdminTable: React.FC = () => {
  ...
  const users = useBenefitCategoryStore((s) => s.benefitCategories);
  const setUsers = useBenefitCategoryStore((s) => s.setBenefitCategories);
  const confirmModal = useModal();
  const [selectedRoleId, setSelectedRoleId] = useState<string | null>(null);
  const [editingRole, setEditingRole] = useState<BenefitCategory | null>(null);
  const removeUser = useBenefitCategoryStore((s) => s.removeBenefitCategory);
  const [errorMessage, setErrorMessage] = useState(
    "Failed to delete category. Please try again."
  );

  type Header = {
    key: keyof BenefitCategory | "actions";
    label: string;
  };
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const url = `/admin/benefit-categories/list?${params.toString()}`;
  ...
  const handleView = (role: BenefitCategory) => {
    setEditingRole(role);
    openModal();
  };

  const handlenavigate = (benefit: BenefitCategory) => {
  ...
  const deleteRole = async () => {
    ...
    try {
      setLoading(true);
      const url = `/admin/benefit-categories/${selectedRoleId}`;
      await apiClient(url, {
        method: "DELETE",
        onLoading: (l) => setLoading(l),
      });
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full table-auto">
            ...
            <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
              {users.map((invoice: BenefitCategory) => (
                <tr
                  key={invoice.id}
