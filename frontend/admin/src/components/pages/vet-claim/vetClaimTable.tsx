...
// Statuses to filter OUT (only show claims NOT in these statuses)
...
const VetClaimTable: React.FC = () => {
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const data = await fetchClaims({
        ...
        // Don't pass status filter - we'll filter client-side
      });
      ...
      const filteredClaims = items.filter(
        (claim) => !EXCLUDED_STATUSES.includes(claim.status)
      );

      setClaims(filteredClaims);
      setTotalItems(filteredClaims.length);
      setHasNextPage(false); // Simplified for filtered data
  ...
  }, [limit, currentPage, search, selectedProviderId, setClaims]);
  ...
  const handleNavigate = (claim: Claim) => {
    router.push(`/vet-claims/${claim.id}`);
  };
