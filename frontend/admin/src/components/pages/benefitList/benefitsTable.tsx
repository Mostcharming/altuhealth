...
const BenefitsTable: React.FC = () => {
  ...
  const [errorMessage, setErrorMessage] = useState(
    "Failed to delete benefit. Please try again."
  );
  const users = useBenefitStore((s) => s.benefits);
  const setUsers = useBenefitStore((s) => s.setBenefits);
  const removeUser = useBenefitStore((s) => s.removeBenefit);
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      if (selectedCategoryId)
        params.append("benefitCategoryId", selectedCategoryId);

      const url = `/admin/benefits/list?${params.toString()}`;
      ...
      console.warn("Benefits fetch failed", err);
    } finally {
      setLoading(false);
    }
  ...
  const deleteBenefit = async () => {
    ...
    try {
      setLoading(true);
      const url = `/admin/benefits/${selectedBenefitId}`;
      await apiClient(url, {
        method: "DELETE",
        onLoading: (l) => setLoading(l),
      });
      removeUser(selectedBenefitId);
      setSelectedBenefitId(null);
      confirmModal.closeModal();
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                >
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => handleDeleteModal(benefit.id)}
      ...
      <ConfirmModal
        ...
        handleSave={deleteBenefit}
        closeModal={handleCloseConfirm}
      />
      <EditBenefit
        isOpen={isOpen}
        closeModal={handleCloseEdit}
        benefit={editingBenefit as any}
      />
      <SuccessModal
