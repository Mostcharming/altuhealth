...
const ReferrersTable = () => {
  ...
  const [errorMessage, setErrorMessage] = useState(
    "Failed to delete referrer. Please try again."
  );
  ...
  const fetchReferrers = useCallback(async () => {
    try {
      ...
      const data = await referrerAPI.listReferrers({
        page: currentPage,
        limit,
        q: search,
      ...
      const items: Referrer[] =
        data?.data?.referrers && Array.isArray(data.data.referrers)
          ? data.data.referrers
          : Array.isArray(data)
          ? data
          : [];
  ...
  }, [currentPage, limit, search, selectedStatus]);
  ...
  const deleteReferrer = async () => {
    ...
    try {
      setLoading(true);
      await referrerAPI.deleteReferrer(selectedReferrerId);

      setReferrers((prev) => prev.filter((r) => r.id !== selectedReferrerId));
        ...
        <div className="flex gap-3.5">
          <div className="hidden flex-col gap-3 sm:flex sm:flex-row sm:items-center">
            <select
              ...
              className="h-11 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
            >
                ...
                referrers.map((referrer) => (
                  ...
                  >
                    ...
                    <td className="p-4 whitespace-nowrap text-center">
                      <p
                        className={`text-xs rounded-full px-2 py-0.5 font-medium ${getStatusBadgeColor(
                          referrer.status
                        )}`}
                      >
                    ...
                    <td className="p-4 whitespace-nowrap">
                      <div className="flex items-center w-full gap-2">
                        <button
                          onClick={() =>
                            router.push(`/referrals/${referrer.id}`)
                          }
