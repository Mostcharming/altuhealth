...
interface RetailEnrolleeDependentsTableProps {
...
const RetailEnrolleeDependentsTable: React.FC<
  RetailEnrolleeDependentsTableProps
> = ({ retailEnrolleeId }) => {
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const url = `/admin/retail-enrollees/${retailEnrolleeId}/dependents?${params.toString()}`;
      ...
      const items: Dependent[] =
        data?.data?.dependents && Array.isArray(data.data.dependents)
      ...
      console.warn("Enrollee dependents fetch failed", err);
    } finally {
      setLoading(false);
    }
  }, [limit, currentPage, retailEnrolleeId]);
            ...
            <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
              {dependents.map((dependent) => (
                ...
                >
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {capitalizeWords(dependent.relationshipToEnrollee || "-")}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
...
export default RetailEnrolleeDependentsTable;
