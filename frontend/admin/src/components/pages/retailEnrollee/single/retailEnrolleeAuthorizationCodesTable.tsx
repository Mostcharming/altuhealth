...
import {
  EnrolleeAuthorizationCode,
  useEnrolleeAuthorizationCodeStore,
} from "@/lib/store/enrolleeAuthorizationCodeStore";
import React, { useCallback, useEffect, useState } from "react";

interface RetailEnrolleeAuthorizationCodesTableProps {
  retailEnrolleeId: string;
}

type Header = {
  key: keyof EnrolleeAuthorizationCode | "actions";
  label: string;
};

const RetailEnrolleeAuthorizationCodesTable: React.FC<
  RetailEnrolleeAuthorizationCodesTableProps
> = ({ retailEnrolleeId }) => {
  ...
  const authorizationCodes = useEnrolleeAuthorizationCodeStore(
    (s) => s.authorizationCodes
  );
  const setAuthorizationCodes = useEnrolleeAuthorizationCodeStore(
    (s) => s.setAuthorizationCodes
  );
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const url = `/admin/retail-enrollees/${retailEnrolleeId}/authorization-codes?${params.toString()}`;
  ...
  }, [limit, currentPage, search, retailEnrolleeId, setAuthorizationCodes]);
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full table-auto">
            ...
            <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
              {authorizationCodes.map((code: EnrolleeAuthorizationCode) => (
                <tr
                  key={code.id}
                ...
                >
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {code.validTo ? formatDate(code.validTo) : "-"}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <span
...
export default RetailEnrolleeAuthorizationCodesTable;
