...
import {
  RetailEnrollee,
  useRetailEnrolleeStore,
} from "@/lib/store/retailEnrolleeStore";
...
const RetailEnrolleeTable: React.FC = () => {
  ...
  const retailEnrollees = useRetailEnrolleeStore((s) => s.retailEnrollees);
  ...
  type Header = {
    key: keyof RetailEnrollee | "actions";
    label: string;
  };
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const data = await fetchRetailEnrollees({
        limit,
        page: currentPage,
        q: search,
      ...
      console.warn("Retail Enrollee fetch failed", err);
    } finally {
      setLoading(false);
    }
  }, [limit, currentPage, search, selectedPlanId, setRetailEnrollees]);
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full table-auto">
              ...
              <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
                {retailEnrollees.map((enrollee: RetailEnrollee) => (
                  <tr
                    key={enrollee.id}
                    ...
                    <td className="p-4 whitespace-nowrap">
                      <div className="flex items-center gap-2 min-w-[120px]">
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-400">
                          {capitalizeAllWords(enrollee.policyNumber)}
                        </span>
...
export default RetailEnrolleeTable;
