...
import { fetchNotificationLogs } from "@/lib/apis/notificationLog";
import capitalizeWords from "@/lib/capitalize";
import { formatDate } from "@/lib/formatDate";
import {
  NotificationLog,
  useNotificationLogStore,
} from "@/lib/store/notificationLogStore";
...
const NotificationLogTable: React.FC = () => {
  ...
  const notificationLogs = useNotificationLogStore((s) => s.notificationLogs);
  const setNotificationLogs = useNotificationLogStore(
    (s) => s.setNotificationLogs
  );

  type Header = {
    key: keyof NotificationLog | "actions";
    label: string;
  };
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      setHasNextPage(Boolean(data?.data?.hasNextPage ?? false));
      setHasPreviousPage(Boolean(data?.data?.hasPrevPage ?? false));
      setTotalPages(data?.data?.totalPages ?? 1);
    } catch (err) {
      console.warn("Notification logs fetch failed", err);
    } finally {
      setLoading(false);
    }
  }, [
  ...
  const handleNotificationTypeChange = (selectedValue: string) => {
              ...
              {notificationLogs.map((log: NotificationLog) => (
                ...
                >
                  <td className="p-4 whitespace-nowrap">
                    ...
                    >
                      {capitalizeWords(log.notificationType || "-")}
                    </span>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                  ...
                  <td className="p-4">
                    <span className="text-sm text-gray-700 dark:text-gray-400 line-clamp-2">
                      {
                        //   log.message ||
