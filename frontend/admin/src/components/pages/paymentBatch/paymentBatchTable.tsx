...
const PaymentBatchTable: React.FC = () => {
  ...
  const paymentBatches = usePaymentBatchStore((s) => s.paymentBatches);
  const setPaymentBatches = usePaymentBatchStore((s) => s.setPaymentBatches);
  const confirmModal = useModal();
  const [selectedBatchId, setSelectedBatchId] = useState<string | null>(null);
  const [editingBatch, setEditingBatch] = useState<PaymentBatch | null>(null);
  const removePaymentBatch = usePaymentBatchStore((s) => s.removePaymentBatch);
  const [errorMessage, setErrorMessage] = useState(
    "Failed to delete payment batch. Please try again."
  );
  ...
  const headers: Header[] = [
    ...
    { key: "numberOfProviders", label: "Providers" },
    { key: "status", label: "Status" },
    { key: "totalClaimsAmount", label: "Claims Amount" },
    { key: "reconciliationAmount", label: "Reconciliation Amount" },
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const url = `/admin/payment-batches/list?${params.toString()}`;
      ...
      console.warn("Payment batch fetch failed", err);
    } finally {
      setLoading(false);
    }
  }, [limit, currentPage, search, setPaymentBatches]);
  ...
  const deletePaymentBatch = async () => {
    if (!selectedBatchId) return;
    try {
      setLoading(true);
      const url = `/admin/payment-batches/${selectedBatchId}`;
      await apiClient(url, {
        method: "DELETE",
        onLoading: (l) => setLoading(l),
      });

      removePaymentBatch(selectedBatchId);
      setSelectedBatchId(null);
      confirmModal.closeModal();
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full table-auto">
            ...
            <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
              {paymentBatches.map((batch: PaymentBatch) => (
                <tr
                  key={batch.id}
                ...
                >
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {capitalizeWords(batch.title)}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {batch.numberOfProviders || 0}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <span
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {typeof batch.totalClaimsAmount === "number"
                        ? `$${batch.totalClaimsAmount.toFixed(2)}`
                        : "$0.00"}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {typeof batch.reconciliationAmount === "number"
                        ? `$${batch.reconciliationAmount.toFixed(2)}`
                        : "$0.00"}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {batch.isPaid ? (
                        <span className="inline-block px-2 py-1 bg-success-100 text-success-700 dark:bg-success-900/30 dark:text-success-400 rounded text-xs font-medium">
                          Yes
                        </span>
                      ) : (
                        <span className="inline-block px-2 py-1 bg-error-100 text-error-700 dark:bg-error-900/30 dark:text-error-400 rounded text-xs font-medium">
                          No
                        </span>
                      )}
                    </p>
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <div className="flex items-center w-full gap-2">
                      ...
                      <button
                        onClick={() => handleDeleteModal(batch.id)}
...
export default PaymentBatchTable;
