...
const AwaitingPaymentClaimTable: React.FC = () => {
  ...
  // Filter only awaiting_payment status
  const targetStatus = useMemo(() => "awaiting_payment", []);
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      // Filter to only awaiting_payment status
      const filteredClaims = items.filter(
        (claim) => claim.status === targetStatus
      );
      ...
      setTotalPages(Math.ceil(filteredClaims.length / limit));
    } catch (err) {
      console.warn("Claims fetch failed", err);
    } finally {
      setLoading(false);
    }
  }, [limit, currentPage, search, selectedProviderId, targetStatus, setClaims]);
  ...
  const handleNavigate = (claim: Claim) => {
    router.push(`/awaiting-payment/${claim.id}`);
  };
...
export default AwaitingPaymentClaimTable;
