...
// const AdminTable: React.FC = ({ id }) => {
const AdminTable: React.FC<AdminTableProps> = ({ id }) => {
  ...
  const [editingRole, setEditingRole] = useState<Benefit | null>(null);
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      if (id) params.append("benefitCategoryId", String(id));
  ...
  const deleteRole = async () => {
    ...
    try {
      setLoading(true);
      const url = `/admin/benefits/${selectedRoleId}`;
      await apiClient(url, {
        method: "DELETE",
        onLoading: (l) => setLoading(l),
      });
