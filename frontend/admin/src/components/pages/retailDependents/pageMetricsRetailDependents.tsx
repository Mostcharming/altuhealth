...
export default function PageMetricsRetailDependents({
  ...
  // stores
  const addDependent = useRetailEnrolleeDependentStore((s) => s.addDependent);
  ...
  const downloadSampleTemplate = () => {
    ...
    a.href = url;
    a.download = "retail_dependent_bulk_upload_template.csv";
  ...
  const handleBulkUpload = async () => {
    try {
      ...
      formData.append("retailEnrolleeId", bulkRetailEnrolleeId);

      const data = await apiClient(
        "/admin/retail-enrollee-dependents/bulk/create",
        {
          method: "POST",
          formData,
  ...
  const handleSubmit = async () => {
    try {
      ...
      const data = await apiClient("/admin/retail-enrollee-dependents", {
        method: "POST",
        body: payload,
        onLoading: (l: boolean) => setLoading(l),
      });
