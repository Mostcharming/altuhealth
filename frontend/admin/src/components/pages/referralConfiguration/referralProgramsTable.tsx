...
import ViewReferralProgramModal from "./viewReferralProgramModal";
...
type Header = {
  key: keyof ReferralProgram | "actions";
  label: string;
};

const ReferralProgramsTable: React.FC = () => {
  ...
  const [editingProgram, setEditingProgram] = useState<ReferralProgram | null>(
    null
  );
  const [viewingProgram, setViewingProgram] = useState<ReferralProgram | null>(
    null
  );
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const items: ReferralProgram[] =
        data?.data?.programs && Array.isArray(data.data.programs)
          ? data.data.programs
          : Array.isArray(data)
          ? data
          : [];
      ...
      console.warn("Referral programs fetch failed", err);
    } finally {
      setLoading(false);
    }
  ...
  const handleView = (program: ReferralProgram) => {
  ...
  const deleteProgram = async () => {
    ...
    try {
      setLoading(true);
      await referralProgramAPI.deleteProgram(selectedProgramId);
                  ...
                  >
                    ...
                    <td className="p-4 whitespace-nowrap">
                      <p className="text-sm text-gray-700 dark:text-gray-400">
                        {capitalizeWords(program.rewardType)}
                      </p>
                    </td>
                    <td className="p-4 whitespace-nowrap text-center">
                      <p
                        className={`text-xs rounded-full px-2 py-0.5 font-medium ${getStatusColor(
                          program.status
                        )}`}
                      >
                        {capitalizeWords(program.status)}
                      </p>
                    </td>
                    <td className="p-4 whitespace-nowrap">
                      <p className="text-sm text-gray-700 dark:text-gray-400">
...
export default ReferralProgramsTable;
