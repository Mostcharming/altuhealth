...
export default function SupportMetrics() {
  ...
  if (statsLoading) {
    return (
      <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
        {[...Array(4)].map((_, i) => (
          <div
            key={i}
  ...
  return (
    <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
      {metrics.map((metric, index) => (
        ...
        >
          <div
            className={`${metric.bgColor} ${metric.textColor} inline-flex h-14 w-14 items-center justify-center rounded-xl`}
          >
            {metric.icon}
          </div>
          <div className="flex-1">
            <h3 className="text-sm mb-1 font-semibold text-gray-800 dark:text-white/90">
              {metric.value}
            </h3>
