...
export const useTicketStore = create<TicketStore>((set) => ({
  ...
  listTickets: async (options = {}) => {
    ...
    try {
      const endpoint = `/enrollee/tickets?page=${options.page || 1}&limit=${
        options.limit || 10
  ...
  getTicket: async (ticketId, includeMessages = false) => {
    set({ ticketsLoading: true, ticketsError: null });
    try {
      const endpoint = `/enrollee/tickets/${ticketId}${
  ...
  createTicket: async (data) => {
    set({ ticketsLoading: true, ticketsError: null });
    try {
      const response = await apiClient("/enrollee/tickets", {
        method: "POST",
        body: data,
      });
  ...
  updateTicket: async (ticketId, data) => {
    set({ ticketsLoading: true, ticketsError: null });
    try {
      const response = await apiClient(`/enrollee/tickets/${ticketId}`, {
        method: "PUT",
        body: data,
      });
  ...
  getMessages: async (ticketId, options = {}) => {
    ...
    try {
      const endpoint = `/enrollee/tickets/${ticketId}/messages${
  ...
  addMessage: async (ticketId, data) => {
    ...
    try {
      ...
      const response = await apiClient(
        `/enrollee/tickets/${ticketId}/messages`,
        requestOptions
      );
      ...
      const enrichedMessage: TicketMessage = {
        ...
        senderType: (message.senderType ||
          ...
          "Enrollee") as TicketMessage["senderType"],
