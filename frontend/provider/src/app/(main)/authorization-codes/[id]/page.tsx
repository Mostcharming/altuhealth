...
export default function SingleAuthorizationCode() {
  ...
  useEffect(() => {
    const fetchAuthorizationCode = async () => {
      try {
        setLoading(true);
        const response = await apiClient(`/provider/authorization-codes/${id}`);
        setAuthorizationCode(response?.data?.authorizationCode || null);
      } catch (error) {
        setErrorMessage(
  ...
  return (
    <div>
      <PageBreadcrumbSub
        parentTitle="Authorization Codes"
        parentHref="/authorization-codes"
        currentTitle={pageTitle}
      />

      {errorMessage || !authorizationCode ? (
        <div className="mt-6 rounded-lg border border-red-200 bg-red-50 p-5 text-sm text-red-700 dark:border-red-900/30 dark:bg-red-900/10 dark:text-red-400">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="rounded-lg border border-brand-200 bg-brand-50 px-4 py-3 text-right dark:border-brand-900/30 dark:bg-brand-900/10">
                ...
                <p className="mt-1 text-2xl font-semibold text-brand-900 dark:text-brand-200">
                  {money(
                    authorizationCode.amountAuthorized,
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                  {authorizationCode.renderedItems?.length ? (
                    authorizationCode.renderedItems.map((item) => {
                      const itemType = item.drugId ? "Drug" : "Service";
                      return (
                        <tr key={item.id}>
                          <td className="p-4">
