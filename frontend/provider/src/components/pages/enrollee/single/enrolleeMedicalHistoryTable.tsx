...
} from "@/lib/store/enrolleeMedicalHistoryStore";
import React, { useCallback, useEffect, useState } from "react";

interface EnrolleeMedicalHistoryTableProps {
  enrolleeId: string;
}

const EnrolleeMedicalHistoryTable: React.FC<
  EnrolleeMedicalHistoryTableProps
> = ({ enrolleeId }) => {
  ...
  const medicalHistories = useEnrolleeMedicalHistoryStore(
    (s) => s.medicalHistories
  );
  const setMedicalHistories = useEnrolleeMedicalHistoryStore(
    (s) => s.setMedicalHistories
  );

  type Header = {
    key: keyof EnrolleeMedicalHistory | "actions";
    label: string;
  };
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      const data = await fetchEnrolleeMedicalHistories({
        enrolleeId,
        limit,
        page: currentPage,
      ...
      const items: EnrolleeMedicalHistory[] =
  ...
  }, [limit, currentPage, selectedStatus, enrolleeId, setMedicalHistories]);
                ...
                >
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {capitalizeWords(history.Provider?.name || "-")}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {capitalizeWords(history.Diagnosis?.name || "-")}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
...
export default EnrolleeMedicalHistoryTable;
