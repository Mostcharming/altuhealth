...
import {
  AdmissionTracker,
  useAdmissionTrackerStore,
} from "@/lib/store/admissionTrackerStore";
import { Provider, useProviderStore } from "@/lib/store/providerStore";
import React, { useCallback, useEffect, useState } from "react";
import AdmissionTrackerDetailModal from "./AdmissionTrackerDetailModal";

const AdmissionTrackerTable: React.FC = () => {
  ...
  type Header = {
    key: keyof AdmissionTracker | "actions";
    label: string;
  };
  ...
  const fetch = useCallback(async () => {
    try {
      ...
      console.warn("Admissions fetch failed", err);
    } finally {
      setLoading(false);
    }
  }, [
  ...
  const handleViewDetails = (admission: AdmissionTracker) => {
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full table-auto">
            ...
            <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
              {admissions.map((admission: AdmissionTracker) => (
                <tr
                  key={admission.id}
                ...
                >
                  <td className="p-4 whitespace-nowrap">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-400">
                      {admission.id?.substring(0, 8) || "-"}
                    </span>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-400">
                      {admission.Enrollee
                        ? `${admission.Enrollee.firstName} ${admission.Enrollee.lastName}`
                        : "-"}
                    </span>
                  </td>
                  <td className="p-4 whitespace-nowrap">
                  ...
                  <td className="p-4 whitespace-nowrap">
                    <p className="text-sm text-gray-700 dark:text-gray-400">
                      {admission.diagnosis
                        ? admission.diagnosis.substring(0, 30) + "..."
                        : "-"}
                    </p>
                  </td>
                  <td className="p-4 whitespace-nowrap">
...
export default AdmissionTrackerTable;
